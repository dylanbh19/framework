...
class RobustCallCenterPipeline:
    """
    End-to-end pipeline for call-center data, from raw dumps
    to modelling-ready feature tables and business-grade reports.
    """
    ...
    def load_data(self, genesys_df=None, contact_df=None):
        ...
        try:
            # — From DataFrames passed directly —
            if genesys_df is not None and contact_df is not None:
                self.genesys_df, self.contact_df = genesys_df.copy(), contact_df.copy()
            else:
                # — From files —
                if self.genesys_path:
                    self.genesys_df = (
    ...
    def clean_connection_ids(self, df: pd.DataFrame, col_name: str) -> pd.DataFrame:
        """
        Return df with *_clean1 … *_clean6 variants.

        Strategy 1 now removes the **exact prefix `GTest`**
        (any case) and the **suffix `_1`** before anything
        else, per the latest requirements.                    # <<< CHANGED
        """
        if col_name not in df.columns:
            return df
        ...
        # -- Strategy 1: strip GTest & _1, plus generic prefixes/suffixes --
        df[f"{col_name}_clean1"] = (
            df[col_name]
            .str.replace(r"(?i)^gtest", "", regex=True)  # remove 'GTest' prefix  # <<< CHANGED
            .str.replace(r"_1$", "", regex=True)         # remove '_1' suffix     # <<< CHANGED
            .str.replace(r"^00E303907400", "", regex=True)
        ...
        # -- Strategy 5: ensure a leading zero if none present ---------- (unchanged)
        def _ensure_leading_zero(token):
            if pd.isna(token):
                return token
            s = str(token)
        ...
        # -- Strategy 6: grab last 10 digits (fallback) -----------------
        df[f"{col_name}_clean6"] = (
            df[f"{col_name}_clean4"].str.extract(r"(\d{10})$")  # may be NaN
        )
    ...
    def clean_and_standardize_data(self):
        ...
        # >>> NEW: **only keep refs that, after stripping, start with “00”**  # <<< CHANGED
        keep_mask = self.contact_df[f"{c_ref}_clean5"].str.startswith("00", na=False)
        removed = (~keep_mask).sum()
        if removed:
            print(f"  • Dropping {removed:,} contact rows that do not start with '00'")
        self.contact_df = self.contact_df[keep_mask].reset_index(drop=True)
    ...
    def create_contact_aggregations(self):
        ...
        for col in [
            self.find_column(self.contact_df, self.column_mappings["call_centre"]),
            "CallerType",
            "CompanyCode",
        ]:
            if col and col in self.contact_df.columns:
        ...
        contact_aggs = []
        for clean_col in [ref_col] + [f"{ref_col}_clean{i}" for i in range(1, 7)]:  # <<< CHANGED
            if clean_col in self.contact_df.columns:
                try:
                    tmp = (
                        self.contact_df.groupby(clean_col)
    ...
    def merge_datasets_robust(self):
        ...
        for g_suffix in [""] + [f"_clean{i}" for i in range(1, 7)]:  # <<< CHANGED
            g_col = f"{g_conn}{g_suffix}"
            for c_suffix in [""] + [f"_clean{i}" for i in range(1, 7)]:  # <<< CHANGED
                c_col = (
                    f"{self.contact_agg.columns[0]}{c_suffix}" if c_suffix else self.contact_agg.columns[0]
                )
                if g_col in self.genesys_df.columns and c_col in self.contact_agg.columns:
                    merge_candidates.append((g_col, c_col))
    ...
    # ------------------------------------------------------------------
    # The rest of the pipeline is **unchanged** from the previous version
    # (time metrics, temporal features, intent augmentation, etc.)
    # ------------------------------------------------------------------
